...
def core_is_usable(core:Path, version:str)->bool:
    ...
    expected=version.lstrip('v');return expected in (cp.stdout or '')

def quarantine_core(core:Path)->Path:
    stamp=time.strftime('%Y%m%d-%H%M%S');target=core.with_name(core.name+f'.broken-{stamp}');n=1
    while target.exists():target=core.with_name(core.name+f'.broken-{stamp}-{n}');n+=1
    core.rename(target);return target

def owner_password()->str:
    if not sys.stdin.isatty():raise SystemExit('Interactive TTY is required for owner password entry')
    while True:
        try:
            first=getpass.getpass(f'DARK owner password ({PASSWORD_MIN_LENGTH}-{PASSWORD_MAX_LENGTH} characters): ');second=getpass.getpass('Repeat owner password: ')
        ...
        if not PASSWORD_MIN_LENGTH<=len(first)<=PASSWORD_MAX_LENGTH:
            print(f'Password must be between {PASSWORD_MIN_LENGTH} and {PASSWORD_MAX_LENGTH} characters. Try again.');continue
        if first!=second:print('Passwords do not match. Try again.');continue
        return first
...
def main():
    ...
    if APP.exists() or CONF.exists() or DATA.exists():raise SystemExit('Existing DARK paths found. Nothing overwritten. Back up before an explicit upgrade; this is the fresh installer.')
    ...
    if a.install_os_packages:run(['apt-get','update']);run(['apt-get','install','-y','python3-venv','ca-certificates'])
    try:account=pwd.getpwnam('darkxray')
    except KeyError:run(['useradd','--system','--home-dir',str(DATA),'--shell','/usr/sbin/nologin','darkxray']);account=pwd.getpwnam('darkxray')
    ...
    for name in ['backend','web','tools','deploy']:shutil.copytree(ROOT/name,APP/name,ignore=shutil.ignore_patterns('__pycache__','*.pyc'))
    for name in ['darkxray','requirements.txt','LICENSE','THIRD-PARTY-NOTICES.md','VERSION']:shutil.copy2(ROOT/name,APP/name)
    run([sys.executable,'-m','venv',str(APP/'.venv')]);py=APP/'.venv/bin/python';run([py,'-m','pip','install','-q','--disable-pip-version-check','-r',APP/'requirements.txt'])
    ...
    if core.exists() and core_is_usable(core,a.core_version):print(f'Reusing verified existing Xray core: {core}')
    else:
        if core.exists():print(f'Incomplete/mismatched Xray core moved aside: {quarantine_core(core)}')
        if a.core_archive:run([py,APP/'tools/import-core.py','--archive',a.core_archive.resolve(),'--sha256',a.core_sha256,'--destination',core])
