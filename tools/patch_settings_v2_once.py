...
old="SECTIONS = {'outbounds','routing','dns','policy','observatory','hosts','panel','ipguard'}"
new="SECTIONS = {'outbounds','routing','dns','policy','observatory','hosts','panel','runtime','subscription','ipguard'}"
...
old="""        return {'saved':True,'applied':False,'runtime':self.runtime_state()}
"""
new="""        result={'saved':True,'applied':False,'runtime':self.runtime_state()}
...
old="""    def subscription(self,email:str,fmt:str)->tuple[bytes,dict]:
        ...
        return body,headers
"""
new="""    def subscription(self,email:str,fmt:str)->tuple[bytes,dict]:
        if fmt not in ('raw','base64'):raise CoreError('This release exports raw/base64; other formats are not silently substituted',status=400)
...
old="""        response=JSONResponse({'id':p.actor.id,'role':p.actor.role,'csrf':p.csrf,'permissions':p.actor.permissions})
        response.set_cookie(COOKIE,token,max_age=8*3600,httponly=True,secure=config.secure_cookie,samesite='strict',path='/')
"""
new="""        response=JSONResponse({'id':p.actor.id,'role':p.actor.role,'csrf':p.csrf,'permissions':p.actor.permissions})
...
"""
assert old in s;s=s.replace(old,new,1)
old="""            'poll_seconds':config.poll_seconds,'engine_version':engine.version,'independent':True,'test_engine':config.test_engine}
"""
new="""            'poll_seconds':config.poll_seconds,'engine_version':engine.version,'independent':True,'test_engine':config.test_engine,
            'ui':engine.section('panel')}
"""
assert old in s;s=s.replace(old,new,1)
old="""        fmt=request.query_params.get('format','base64')
...
p=Path('darkxray');s=p.read_text()
old=""" guard-enable) shift; exec \"$PY\" \"$ROOT/tools/guard-enable.py\" \"$@\" ;;
 selftest) shift; exec \"$PY\" \"$ROOT/tools/smoke-real.py\" \"$@\" ;;
"""
new=""" guard-enable) shift; exec \"$PY\" \"$ROOT/tools/guard-enable.py\" \"$@\" ;;
...
old="""  4) Reset owner password
  5) Show access URL / SSH tunnel
  0) Back''')
"""
new="""  4) Reset owner password
  5) Show access URL / SSH tunnel
...
p=Path('web/index.html');s=p.read_text();s=s.replace('<link rel="stylesheet" href="/assets/inbounds-v3.css">','<link rel="stylesheet" href="/assets/inbounds-v3.css"><link rel="stylesheet" href="/assets/settings-v2.css">');s=s.replace('<script defer src="/assets/runtime-policy.js"></script>','');s=s.replace('<script defer src="/assets/forms06.js"></script>','<script defer src="/assets/forms06.js"></script><script defer src="/assets/settings-v2.js"></script>');p.write_text(s)
p=Path('tests/run-tests.sh');s=p.read_text();needle="python -m pytest tests/test_v06.py -q --junitxml=qa/junit/v06.xml\n";assert needle in s;s=s.replace(needle,needle+"python -m pytest tests/test_settings_v2.py -q --junitxml=qa/junit/settings-v2.xml\n",1);p.write_text(s)
p=Path('.github/workflows/ci.yml');s=p.read_text();s=s.replace('python -m py_compile tools/menu.py tools/update.py tools/provision.py backend/server.py backend/policy_auth.py','python -m py_compile tools/menu.py tools/update.py tools/provision.py tools/settings_apply.py backend/server.py backend/policy_auth.py backend/reality_scan.py');s=s.replace('node --check web/inbounds-v3.js','node --check web/inbounds-v3.js\n          node --check web/settings-v2.js');s=s.replace("          grep -q '/assets/runtime-policy.js' web/index.html\n",'');s=s.replace("          grep -q '/assets/inbounds-v3.js' web/index.html\n", "          grep -q '/assets/inbounds-v3.js' web/index.html\n          grep -q '/assets/settings-v2.css' web/index.html\n          grep -q '/assets/settings-v2.js' web/index.html\n          grep -q \"settings-apply\" darkxray\n");p.write_text(s)
