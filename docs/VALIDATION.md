...
- `nodes-v3-assignment`: regressions cover per-node inbound assignment, Central sync sending only selected inbound definitions and only client credentials attached to those inbounds, remote Agent mirror reconciliation, deselection cleanup, and replacement of manual clone UI with assignment + sync controls. Remote traffic accounting is intentionally not claimed as globally converged.
