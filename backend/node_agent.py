...
def make_agent_app(engine:CoreEngine,store:Store,token:AgentToken,node_id:str,*,background:bool=True)->FastAPI:
    ...
    def health(_scope:str=Depends(auth)):
        ...
        return {'service':'DARK XRAY NODE','agent_only':True,'version':VERSION,'node_id':node_id,'installed_source':source,
                ...
                'installation_id':runtime.installation_id,'capabilities':{'credential_rotation':1,'ordered_control':1,'installation_identity':1,'replacement_prepare':1,'conditional_activation':1,'guard_status':1,'outbound_probe':1,'warp_endpoint_probe':1},'control_receipt':runtime.command_status(),
    ...
    def outbound_probe(body:dict,_scope:str=Depends(auth)):
        ...
        attempts=body.get('attempts',1) if isinstance(body,dict) else 1
        timeout=body.get('timeoutSeconds',5) if isinstance(body,dict) else 5
        if not isinstance(tags,list) or len(tags)>32 or any(not isinstance(x,str) or not x or len(x)>128 for x in tags):
            raise HTTPException(400,'Invalid outbound probe tags')
        if type(attempts) is not int or not 1<=attempts<=3 or type(timeout) is not int or not 1<=timeout<=10:
            raise HTTPException(400,'Invalid outbound probe limits')
        outbounds=engine.section('outbounds')
        selected=tags or [str(x.get('tag')) for x in outbounds if isinstance(x,dict) and x.get('tag')]
        try:
            items=probe_outbounds(engine._binary(),engine.config.xray_assets,outbounds,tags=selected,
                                  attempts=attempts,timeout=float(timeout))
        except OutboundProbeError as ex:
            raise HTTPException(422,str(ex))
        return {'service':'DARK XRAY NODE','nodeId':node_id,'items':items,
    ...
    def warp_endpoint_probe(body:dict,_scope:str=Depends(auth)):
        tag=str(body.get('tag') or 'warp') if isinstance(body,dict) else 'warp'
        ...
        if not tag or len(tag)>128 or type(attempts)is not int or not 1<=attempts<=3 or type(timeout)is not int or not 1<=timeout<=10:
            raise HTTPException(400,'Invalid WARP endpoint probe request')
        outbounds=engine.section('outbounds');outbound=next((x for x in outbounds if isinstance(x,dict) and x.get('tag')==tag),None)
        if not outbound or str(outbound.get('protocol','')).lower()!='wireguard':
            raise HTTPException(400,'Unknown WARP outbound')
        ...
        raw_endpoints=endpoints if endpoints is not None else warp_endpoint_candidates(current)
        if not isinstance(raw_endpoints,list) or not 1<=len(raw_endpoints)<=20:
            raise HTTPException(400,'Select 1-20 WARP endpoints')
        clean=[]
        try:
        ...
        for index,endpoint in enumerate(clean):
            item=copy.deepcopy(outbound);probe_tag=f'dark-node-warp-{index}'
            item['tag']=probe_tag;item['settings']['peers'][0]['endpoint']=endpoint
            clones.append(item);mapping[probe_tag]=endpoint
        try:
            rows=probe_outbounds(engine._binary(),engine.config.xray_assets,clones,
                                 tags=[x['tag'] for x in clones],attempts=attempts,timeout=float(timeout),trace=True)
        except OutboundProbeError as ex:
            raise HTTPException(422,str(ex))
        for row in rows:row['endpoint']=mapping.get(str(row.get('tag') or ''),'')
        return {'service':'DARK XRAY NODE','nodeId':node_id,'current':current,'items':rows,
                'productionTrafficMutation':False}
