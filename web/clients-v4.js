...
async function clientFormV4(email=null,inbound=null){
 ...
 const body=`<div class="cv4-editor-shell">
  ...
  <section class="cv4-edit-section"><header><span>03</span><div><b>${L('Plan','پلن')}</b><small>${L('Quota, expiry and IP limit.','حجم، انقضا و محدودیت IP.')}</small></div></header><div class="cv4-plan-grid"><div class="cv4-choice"><span>${L('Traffic','حجم')}</span>${segment('planMode',[['unlimited',L('Unlimited','نامحدود')],['limited',L('Limited','حجمی')]],plan)}</div><div data-cv4-quota class="${plan==='limited'?'':'hidden'}">${fInput(L('Quota (GiB)','حجم GiB'),'totalGB',plan==='limited'?(Number(c.totalGB||0)/gb):50,'number','min="0.01" step="0.01"')}</div><div class="cv4-choice"><span>${L('Expiry','انقضا')}</span>${segment('expiryMode',[['unlimited',L('Unlimited','نامحدود')],['days',L('Days','روز')],['date',L('Date','تاریخ')]],expiryMode)}</div><div data-cv4-days class="hidden">${fInput(L('Days from now','تعداد روز'),'expiryDays',30,'number','min="1" max="36500"')}</div><div data-cv4-date class="${expiryMode==='date'?'':'hidden'}">${fInput(L('Expiry date','تاریخ انقضا'),'expiry',dateValue,'datetime-local')}</div>${fInput(L('IP limit · 0 unlimited','محدودیت IP'),'limitIp',c.limitIp??1,'number','min="0" max="1000" required')}${fInput(L('HWID limit · 0 unlimited','محدودیت HWID'),'limitHwid',c.limitHwid??0,'number','min="0" max="1000" required')}</div></section>
