...
function row(r){
 ...
 return `<article class="cv4-row">
  ...
  <div class="cv4-expiry"><b>${e(expiry)}</b><small>IP ${c.limitIp||'∞'} · HWID ${c.limitHwid||'∞'}</small></div>
  <div class="cv4-row-actions">${mini(L('OPEN','بازکردن'),'cv4detail',r.email,'open')}${can('clients.credentials',r.owner)?mini('QR','cv3links',r.email):''}${mini('•••','cv4more',r.email,'more')}</div>
 </article>`;
}
function stats(){
...
async function detail(id){
 ...
 dialog(L('Client command','فرمان کاربر')+' · '+id,`<div class="cv4-detail"><header><span class="cv4-avatar big">${e(id.slice(0,1).toUpperCase())}</span><div><small>${e(r.owner)} · ${e(c.group||L('Ungrouped','بدون گروه'))}</small><h2>${e(id)}</h2></div>${signal(r)}</header><div class="cv4-detail-grid"><section><span>${L('USAGE','مصرف')}</span><b>${bytes(used)} / ${total?bytes(total):L('Unlimited','نامحدود')}</b>${total?`<div class="cv4-meter"><i style="width:${pct(used,total)}%"></i></div>`:''}</section><section><span>${L('EXPIRY','انقضا')}</span><b>${e(expiry)}</b><small>IP ${c.limitIp||'∞'} · HWID ${c.limitHwid||'∞'}</small></section><section class="wide"><span>${L('SERVICE','سرویس')}</span><div class="cv4-tags">${ins.map(x=>`<i>${e(x)}</i>`).join('')}</div></section></div><footer>${can('clients.credentials',r.owner)?button(L('QR & Links','QR و لینک'),'cv3links','link',`data-id="${e(id)}"`,true):''}${can('clients.edit',r.owner)?button(L('Edit','ویرایش'),'cv4edit','edit',`data-id="${e(id)}"`):''}${can('clients.ip',r.owner)?button('IP / HWID','cv4ips','shield',`data-id="${e(id)}"`):''}${button(L('More actions','عملیات بیشتر'),'cv4more','settings',`data-id="${e(id)}"`)}</footer></div>`);document.querySelector('#overlay .dialog')?.classList.add('cv4-detail-dialog');
}
function more(id){
 const r=state.clients.find(x=>x.email===id);if(!r)return;
 const c=r.client||{};
...
runAction=async function(act,el){
 ...
 if(act==='cv4more')return more(id);
 if(act==='cv4bulk')return bulkCreate();
 if(act==='cv4edit'){closeDialog();return clientFormV4(id);}
 if(act==='cv4ips'){closeDialog();return baseRunAction('ips',{dataset:{id}});}
 if(act==='cv4toggle'){closeDialog();return baseRunAction('toggle',{dataset:{id}});}
 if(act==='cv4resettraffic'){closeDialog();return baseRunAction('reset',{dataset:{id}});}
 if(act==='cv4delete'){closeDialog();return baseRunAction('delete',{dataset:{id}});}
