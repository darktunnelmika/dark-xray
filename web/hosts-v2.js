...
function modePicker(current){return `<div class="hv3-modes">${[['direct',L('Direct','مستقیم'),L('Public IP/domain, inherit inbound transport and security.','IP/دامنه عمومی با تنظیمات خود اینباند.')],['tunnel',L('Tunnel / CDN','تانل / CDN'),L('Change public endpoint and client-facing SNI/Host/Path.','نقطه اتصال بیرونی و SNI/Host/Path مشتری را تنظیم کن.')],['advanced',L('Advanced override','بازنویسی پیشرفته'),L('Only for different TLS/security or format-specific behavior.','فقط برای TLS/امنیت متفاوت یا رفتار فرمت‌ها.')]].map(([v,a,b])=>`<label class="${current===v?'active':''}"><input type="radio" name="mode" value="${v}" ${current===v?'checked':''}><span><b>${a}</b><small>${b}</small></span></label>`).join('')}</div>`;}
...
async function editHost(index=null,cloneMode=false){
 ...
 dialog(index==null?L('New public endpoint','Public نقطه اتصال جدید'):L('Edit public endpoint','ویرایش Public Endpoint'),editorHTML(h,ib0,mode,excluded),async form=>{
