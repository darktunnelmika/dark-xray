...
function endpointMode(h,ib){
 ...
 if(h?.endpointType==='tunnel')return'tunnel';
...
function nodeForRuntime(runtime){const id=String(runtime||'').startsWith('node:')?String(runtime).slice(5):'';return state.hv2.nodes.find(n=>String(n.id)===id)||null;}
...
function normalizeEndpoint(raw,ib){
 ...
 const m=inboundMeta(ib),mode=raw.mode||'direct',endpointTypeValue=mode==='tunnel'?'tunnel':'direct',requestedSecurity=mode==='advanced'&&['same','tls','none'].includes(raw.security)?raw.security:'same',effectiveSecurity=requestedSecurity==='same'?m.security:requestedSecurity;
 ...
 let out={inboundId:Number(ib.id),runtime,endpointType:endpointTypeValue,address,port,remark:String(raw.remark||'').trim(),security:'same',sni:'',overrideSniFromAddress:false,keepSniBlank:false,host:'',path:'',alpn:'',fingerprint:'',allowInsecure:false,finalMask:'',mihomoIpVersion:'',excludeFromSubTypes:[],enable:raw.enable!==false};
 ...
 if(mode==='advanced'){
   ...
   if(effectiveSecurity==='none'){out.sni='';out.overrideSniFromAddress=false;out.keepSniBlank=false;out.alpn='';out.fingerprint='';out.allowInsecure=false;}
...
async function hostsPage(){
 ...
 `<div class="hv2"><div class="notice">${L('Inbound = what Xray listens on. Public Endpoint = what the customer connects to. They can be the same for direct service or different for tunnels/CDNs. A Tunnel/CDN endpoint targeting a Node automatically keeps a Direct sibling for that Node in the same subscription.','اینباند چیزی است که Xray روی آن گوش می‌دهد؛ نقطه اتصال عمومی چیزی است که مشتری به آن وصل می‌شود. در اتصال مستقیم می‌توانند یکی باشند و در تانل/CDN متفاوت. اگر Tunnel/CDN را روی یک Node بسازی، مسیر Direct همان Node هم خودکار در همان ساب نگه داشته می‌شود.')}</div><article class="panel hv3-toolbar"><div class="hv2-toolbar"><input id="hv2search" class="field-input" type="search" placeholder="${L('Search endpoint / address / SNI','جستجوی نقطه اتصال / آدرس / SNI')}" value="${e(state.hv2.search)}"><div class="hv2-filter"><button class="${state.hv2.inbound==='all'?'active':''}" data-act="hv2filter" data-id="all">${L('All','همه')}</button>${state.inbounds.map(i=>`<button class="${String(state.hv2.inbound)===String(i.id)?'active':''}" data-act="hv2filter" data-id="${i.id}">${e(i.remark||i.tag)}</button>`).join('')}</div><div class="spacer"></div><span class="tag">${rows.length}</span></div></article><div class="hv2-grid">${rows.length?rows.map(h=>pageCard(h,h._index)).join(''):empty(L('No public endpoints yet. Without one, DARK falls back to the configured server public address.','هنوز نقطه اتصال عمومی نساخته‌ای. بدون آن، DARK از آدرس عمومی تنظیم‌شده سرور استفاده می‌کند.'))}</div></div>`;
}
...
function editorHTML(h,ib,mode,excluded){
 ...
 return `<div class="hv3-editor"><section class="hv3-step"><header><span>01</span><div><b>${L('Source & mode','مبدا و حالت')}</b><small>${L('Pick the inbound first; DARK shows only controls that can affect its client link.','اول اینباند را انتخاب کن؛ DARK فقط کنترل‌های مؤثر روی لینک همان اینباند را نشان می‌دهد.')}</small></div></header><div class="hv2-form">${select(L('Source inbound','اینباند مبدا'),'inboundId',state.inbounds.map(i=>[i.id,(i.remark||i.tag)+' · '+String(i.protocol).toUpperCase()+' · :'+i.port]),h.inboundId)}${select(L('Runtime','محل اجرا'),'runtime',[['local',L('Main Server','سرور اصلی')],...state.hv2.nodes.map(n=>['node:'+n.id,(n.name||n.id)+' · '+(n.online?L('Online','آنلاین'):L('Offline','آفلاین'))])],h.runtime||'local')}${select(L('Status','وضعیت'),'enable',[['true',L('Active','فعال')],['false',L('Disabled','غیرفعال')]],String(h.enable!==false))}<div class="span-2">${modePicker(mode)}</div></div></section>
 <section class="hv3-step"><header><span>02</span><div><b>${L('Customer endpoint','نقطه اتصال مشتری')}</b><small>${L('This address and port are published to the customer.','این آدرس و پورت داخل لینک مشتری منتشر می‌شوند.')}</small></div></header><div class="hv2-form">${field(L('Public / tunnel address','آدرس عمومی / تانل'),'address',h.address||'','text','required dir="ltr" placeholder="edge.example.com"')}${field(L('Public / tunnel port','پورت عمومی / تانل'),'port',h.port||m.port||443,'number','required min="1" max="65535"')}${field(L('Display name','نام نمایشی'),'remark',h.remark||'','text','maxlength="160"')}<div class="hv3-helper"><button type="button" class="btn" data-act="hv3panelhost">${L('Use panel hostname','استفاده از دامنه پنل')}</button><small>${L('Use only when the panel hostname is also the real customer endpoint.','فقط وقتی دامنهٔ پنل همان نقطه اتصال واقعی مشتری است استفاده کن.')}</small></div></div></section>
 <section class="hv3-step" data-hv3-section="tunnel"><header><span>03</span><div><b>${L('Client-facing route','مسیر سمت مشتری')}</b><small>${L('Inbound transport is inherited. Override only what the tunnel/CDN changes.','انتقال اینباند حفظ می‌شود؛ فقط چیزی را که تانل/CDN عوض می‌کند بازنویسی کن.')}</small></div></header><div class="hv2-form"><div class="span-2 hv3-facts"><span>${L('Inbound','اینباند')}: <b data-hv3-fact="protocol">${e(m.protocol.toUpperCase())}</b></span><span>Transport: <b data-hv3-fact="network">${e(m.network.toUpperCase())}</b></span><span>Security: <b data-hv3-fact="security">${e(m.security.toUpperCase())}</b></span></div><label class="hv3-field" data-hv3-sni><span>SNI</span><select name="sniMode"><option value="inherit" ${sm==='inherit'?'selected':''}>${L('Inherit from inbound','از اینباند')}</option><option value="manual" ${sm==='manual'?'selected':''}>${L('Manual','دستی')}</option><option value="address" ${sm==='address'?'selected':''}>${L('Use endpoint address','از آدرس نقطه اتصال')}</option><option value="blank" ${sm==='blank'?'selected':''}>${L('Keep blank','خالی')}</option></select><small data-hv3-inherited-sni>${baseSni(ib)?L('Inbound SNI: ','SNI اینباند: ')+e(baseSni(ib)):L('Inbound has no SNI.','اینباند SNI ندارد.')}</small></label><div data-hv3-manual-sni>${field(L('Manual SNI','SNI دستی'),'sni',h.sni||'','text','dir="ltr" placeholder="sni.example.com"')}</div><div data-hv3-web>${field('Host','host',h.host||td.host||'','text','dir="ltr"')}</div><div data-hv3-web>${field('Path','path',h.path||td.path||'','text','dir="ltr"')}</div></div></section>
 <section class="hv3-step" data-hv3-section="advanced"><header><span>04</span><div><b>${L('Advanced override','بازنویسی پیشرفته')}</b><small>${L('Use only when the public endpoint terminates a different security layer or needs format-specific behavior.','فقط وقتی نقطه اتصال بیرونی لایهٔ امنیتی متفاوت یا رفتار خاص فرمت‌ها دارد.')}</small></div></header><div class="hv2-form">${select(L('Security published to client','امنیت منتشرشده برای مشتری'),'security',[['same',L('Same as inbound','مثل اینباند')],['tls','TLS'],['none',L('None / plaintext','بدون TLS')]],h.security||'same')}${field('ALPN','alpn',h.alpn||'','text','dir="ltr" placeholder="h2,http/1.1"')}${field(L('Fingerprint','Fingerprint'),'fingerprint',h.fingerprint||'','text','dir="ltr" placeholder="chrome"')}${select(L('Allow insecure TLS','TLS ناامن'),'allowInsecure',[['false',L('No','خیر')],['true',L('Yes','بله')]],String(!!h.allowInsecure))}${select(L('Mihomo IP version','نسخه IP در Mihomo'),'mihomoIpVersion',[['',L('Default','پیش‌فرض')],['dual','dual'],['ipv4','ipv4'],['ipv6','ipv6'],['ipv4-prefer','ipv4-prefer'],['ipv6-prefer','ipv6-prefer']],h.mihomoIpVersion||'')}<label class="span-2 hv3-field"><span>Final Mask JSON</span><textarea class="field-input" name="finalMask" dir="ltr" rows="3" placeholder='{"tcpPadding":true}'>${e(h.finalMask||'')}</textarea><small>${L('Only emitted when configured; leave blank for normal endpoints.','فقط در صورت نیاز منتشر می‌شود؛ برای نقطه اتصال عادی خالی بگذار.')}</small></label><div class="span-2"><label>${L('Exclude this endpoint from formats','عدم نمایش این نقطه اتصال در فرمت‌ها')}</label><div class="check-list"><label><input type="checkbox" name="excludeFormat" value="raw" ${excluded.has('raw')?'checked':''}><span>Raw / Base64</span></label><label><input type="checkbox" name="excludeFormat" value="json" ${excluded.has('json')?'checked':''}><span>DARK JSON</span></label><label><input type="checkbox" name="excludeFormat" value="clash" ${excluded.has('clash')?'checked':''}><span>Clash / Mihomo</span></label></div></div></div></section>
...
function syncEditor(form){
 ...
 form.querySelectorAll('[data-hv3-fact="network"]').forEach(x=>x.textContent=m.network.toUpperCase());
 form.querySelectorAll('[data-hv3-fact="security"]').forEach(x=>x.textContent=m.security.toUpperCase());
 form.querySelectorAll('[data-hv3-inherited-sni]').forEach(x=>x.textContent=baseSni(ib)?L('Inbound SNI: ','SNI اینباند: ')+baseSni(ib):L('Inbound has no SNI.','اینباند SNI ندارد.'));
...
async function editHost(index=null,cloneMode=false){
 ...
 dialog(index==null?L('New public endpoint','نقطه اتصال عمومی جدید'):L('Edit public endpoint','ویرایش نقطه اتصال عمومی'),editorHTML(h,ib0,mode,excluded),async form=>{
   ...
   const v=normalizeEndpoint(raw,ib),node=nodeForRuntime(v.runtime);
   if(v.endpointType==='tunnel'&&!v.remark)v.remark=(node?.name||ib?.remark||L('Endpoint','Endpoint'))+' · '+L('Tunnel','تانل');
   if(index==null||cloneMode)list.push(v);else list[index]=v;
   if(v.endpointType==='tunnel'&&String(v.runtime).startsWith('node:')){
     if(!node?.data_address)throw Error(L('This Node has no client-facing Data Address.','برای این نود Data Address سمت مشتری تعریف نشده است.'));
     const editedIndex=(index!=null&&!cloneMode)?index:-1;
     const directExists=list.some((x,j)=>j!==editedIndex&&Number(x.inboundId)===Number(ib.id)&&String(x.runtime||'local')===v.runtime&&(x.endpointType||endpointMode(x,ib))!=='tunnel');
     if(!directExists)list.push(normalizeEndpoint({mode:'direct',runtime:v.runtime,address:node.data_address,port:Number(ib.port),remark:(node.name||node.id)+' · '+L('Direct','مستقیم'),enable:true},ib));
   }
 ...
 if(form){
   form.addEventListener('change',ev=>{if(ev.target.name==='inboundId'){const ib=inboundFor(ev.target.value);if(ib){form.elements.port.value=ib.port;const td=transportDefaults(ib);if(form.elements.host)form.elements.host.value=td.host;if(form.elements.path)form.elements.path.value=td.path;}}syncEditor(form);});
...
runAction=async function(act,el){
 ...
 if(act==='hv2delete'){const list=(await api('/api/settings/hosts')).value||[];if(confirm(L('Delete this public endpoint? Customer links will stop publishing it.','این نقطه اتصال عمومی حذف شود؟ از لینک‌های مشتری حذف می‌شود.'))){list.splice(Number(el.dataset.index),1);await api('/api/settings/hosts','PUT',{value:list});await refresh();}return;}
...
globalThis.DarkHostV3={inboundMeta,endpointMode,sniMode,normalizeEndpoint,previewModel,transportDefaults,nodeForRuntime};
})();
